    ...
    */
    ...
    if((video.isLoaded())&&(intro)) {
        if(videoState) {
            ...
            m.addStringArg("up");
            senderA.sendMessage(m);
            senderB.sendMessage(m);
            senderC.sendMessage(m);
            videoState = false;
...
void ofApp::loadSettings(){
    ...
    //read IP address from XML
    value = XML.getValue("IP:A", "192.168.1.1");
    senderA.setup(value, PORT);
    cout << "IP A: " << value << endl;
    
    //read IP address from XML
    value = XML.getValue("IP:B", "192.168.1.2");
    senderB.setup(value, PORT);
    cout << "IP B: " << value << endl;
    
    //read IP address from XML
    value = XML.getValue("IP:C", "192.168.1.3");
    senderC.setup(value, PORT);
    cout << "IP C: " << value << endl;
